<script lang="ts">
  ...
  export let gridConfig: IGridProps<any>;

  // TODO:
  // disptach selectedItems, movePage
  // Implement method: go to page
...
>
  <div class="rounded-t mb-0 px-4 py-3 border-0">
    <div class="flex flex-wrap items-center">
      <div
        class="relative w-full px-4 max-w-full flex-grow flex-1 text-center flex justify-between"
      >
        <h3
  ...
  <div class="block w-full overflow-x-auto">
    ...
    <table class="items-center w-full bg-transparent border-collapse">
      <thead>
        <tr>
          {#each gridConfig.columns as column (column.field)}
            <th
          ...
          {/each}
          <th
            class="w-sm px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left {color ===
            'light'
              ? 'bg-blueGray-50 text-blueGray-500 border-blueGray-100'
              : 'bg-red-700 text-red-200 border-red-600'}"
          >
      ...
      <tbody>
        {#each Array(gridConfig.rows) as _, i (i)}
          <tr>
            {#each gridConfig.columns as column (column.field)}
              <td
                class="border-t-0 border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left"
              >
                <span
                  class:font-bold={column.field == "id"}
                  class="ml-3  {color === 'light'
                    ? 'btext-blueGray-600'
                    : 'text-whit'}"
                >
                  {getValue(
                    column.field,
                    gridConfig.dataSource.listaRecords,
                    i
                  )}
                </span>
              </td>
            {/each}
            {#if i <= gridConfig.dataSource.totalRecords - 1}
              <td
                class="border-t-0 px-6 text-center border-l-0 border-r-0 text-xs whitespace-nowrap p-4"
              >
                <TableDropdown
                  actions={gridConfig.actions}
                  index={i}
